...
def move2board(y): 
    '''
    Input:
        - y: a Vec with domain {'y1','y2','y3'},
          the coordinate representation in whiteboard 
    ...
    Output:
        - A {'y1','y2','y3'}-Vec, the coordinate representation
          in whiteboard coordinates of the point p 
          such that the line through the origin 
...
# for v in make_equations(8,25,1,0): (v)
...
w = Vec(D, {('y1', 'x1'): 1, ('y1', 'x2'): 0, ('y1', 'x3'): 0,
            ('y2', 'x1'): 0, ('y2', 'x2'): 0, ('y2', 'x3'): 0,
            ('y3', 'x1'): 0, ('y3', 'x2'): 0, ('y3', 'x3'): 0})
...
def make_nine_equations(corners):
    ...
    input: a list of four tuples:
           ...
           where i0,j0 are the pixel coordinates of the top-left corner, (0,0,1)
                 i1,j1 are the pixel coordinates of the bottom-left corner, (0,1,1)
                 i2,j2 are the pixel coordinates of the top-right corner, (1,0,1)
                 i3,j3 are the pixel coordinates of the bottom-right corner, (1,1,1)
    ...
    e1 = make_equations(corners[0][0], corners[0][1], 0/1, 0/1)
    e2 = make_equations(corners[1][0], corners[1][1], 0/1, 1/1)
    e3 = make_equations(corners[2][0], corners[2][1], 1/1, 0/1)
    e4 = make_equations(corners[3][0], corners[3][1], 1/1, 1/1)
    veclist.append(e1[0])
    veclist.append(e1[1])
    veclist.append(e2[0])
    veclist.append(e2[1])
...
corners = [(358, 36), (329, 597), (592, 157), (580, 483)] # top-left, bottom-left, top-right, bottom-right	
